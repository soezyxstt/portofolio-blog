...
export default function Main({ children }: { children?: React.ReactNode }) {
  ...
  function clamp(num: number, min: number, max: number, tolerance = 0.3) {
  ...
  function onScroll() {
    ...
    if (scroll >= 200 * vh - 10 && scroll < 220 * vh) {
      aboutMe[0].classList.add('opacity-100');
    } else if (scroll >= 150 * vh && scroll < 200 * vh) {
      aboutMe[0].classList.remove('opacity-100');
    }
